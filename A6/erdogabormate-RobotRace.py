...
class MyPathFindingPlayer(Player):
        ...
        def reset(self, player_id, max_players, width, height):
                self.player_name = "erdogabormate"
        ...
        def move(self, status):
                ...
                # numMoves = max(numMoves) for numMoves in range(1,100)
                #    for totalCost in [(distance // numMoves)*cost(numMoves) + cost(distance % numMoves)]
                #                if numMoves==1 or (totalCost < status.gold * 0.2
                #                                   and totalCost < goldInPot * 0.3)
                #)

                if status.gold>50: 
                    numMoves = random.randint(3, 4)
                elif status.gold>30:
                    numMoves = random.randint(2, 3)
                else: 
                    numMoves = 1
                if distance < 6 and status.gold > 10:
                        numMoves = int(5/distance)
                        print("erdogabormate: I jump on it")
                
                ## don't move if the pot is too far away
                if numMoves>0 and distance/numMoves > status.goldPotRemainingRounds:
                        numMoves = 0
                        print("erdogabormate: I rather wait")
                ### HERE we should not zero out but explore if there is anything left
