#!/usr/bin/env python3 
#k-let Shuffling
#called with command "python3 MarkusBeicht-KLetShuffle_2.py -N 5 -k 5 KLetShuffle-test30_k2.in"
...
def Prep(sequence, k):
	...
	return edges_num, dic, n, unique_nodes



#parses keyboard arguments for input N, k and input file
def add_args(parser):
	...
	parser.add_argument("-k", required=True, type=int, help="The option -k specifies the k-let length.")


#calls K-let Shuffle function N times
def main(args):	

	edges, dictionary, n, unique_nodes = Prep(sequence, args.k)
	
	#finds 100 EulerianPaths and saves all unique paths 
	all_paths = []
	i = 100
	while i > 0:
		edges_copy = edges[:]
...
def FindEulerianPath(edges, G, n):
	...
	DFS(FindStartNode(outdeg, indeg), outdeg, G)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generates N sequecnes randomized with the K-let Shuffle approach.")
    add_args(parser)
